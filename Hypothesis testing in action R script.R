...
scores<-read.csv("clipboard", header=T, sep="\t")
## copies the data in, header=T treats the first row as headers not actual data
attach(scores)   ## ataches the data into R
names(scores)    ## allows you to see the coloum headers to check you have imported the correct data 
...
## if you were running a pearsons you would just replace 'spearman' with 'pearson'
...
## some tests will require you to install and load a package to run. You only need to install the first time you use the package
library(FSA) ## loads the library so you can use it, need to load the library each session
...
GPAScore<-read.csv("clipboard", header=T, sep="\t") 
attach(GPAScore)  
names(GPAScore)   
...
## Prediction: Students that spend more time studying will recieve higher exam scores

## load data

Exam<-read.csv("clipboard", header=T, sep="\t")
names(Exam)
attach(Exam)
...
## interpretation
## wilcox test is no significant, therefore there is no significant differences in the two bulb type prices 
...
## Hypothesis: Agibility scores will be different in the three different dog breeds
## Prediction: German shepards and Springer Spaniels will score higher than French Bulldogs in the agility test

dogs<-read.csv("clipboard", header=T, sep="\t") 
attach(dogs)  
names(dogs)   
...
## significant differences in dog breed agility scores, need to run future analysis to find out what breeds the significant differences lie 
