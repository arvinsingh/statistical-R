...
## If you run a line of code with "# then comments" it won't do anything, R knows they are comments
...
income.data<-read.csv("clipboard", header=T, sep="\t")
attach(income.data)   
names(income.data)     
summary(income.data)
...
hist(happiness)
# observaions are roughly bell-shaped, so we can proceed with the linear regression
...
par(mfrow=c(1,1))
##The par(mfrow()) code will divide the plots window in the number of rows and columns specified in the brackets
...
## Prediction: The diameter of the tree will increase as the height of the tree increases 
...
hist(Height)
# observations are roughly bell-shaped, so we can proceed with the linear regression
# Linearity 
...
## for each unit increase in height, girth increases by 1.05 +/- 0.32 (p<0.001 or p<0.05)
## Check for homoscedasticity 
par(mfrow=c(2,2))
plot(trees.lm)
par(mfrow=c(1,1))
##Not as good as the previous model, but this is still alright (bit of a judgement call)
...
heart.data<-read.csv("clipboard", header=T, sep="\t")
attach(heart.data)   
names(heart.data)     
summary(heart.data)
## This gives us a numeric summary table
...
cor(biking, smoking)
## output is 0.015, which means the correlation between biking and smoking is small (0.015 or 1.5%), so we can include both parameters in our model
...
hist(heart.disease)
## the distribution of observations is roughy bell-shaped so we can proceed with the linear regression

## 3.Linearity
## we can check this using two scatterplots (for both independent variables): one for biking and heart disease, and one for smoking and heart disease
...
##as we have count data, and assumptions of lm not met we are justified to use glm with poisson family 
...
##as this is a made up dataset this isn't surprising - can potential fix with running a quasipoisson model
##read up on quasipoisson outside this class - would just add quasipoisson to the family instead of poisson

##Chi-square goodness of fit
pchisq(210.39, 50, lower.tail = FALSE)
##The p-value for this test is is much smaller than the significance level of 0.05. We can conclude that the data doesn't fit the model reasonably well
...
## Load data - need to download data from the link in the powerpoint and open with Rstudios 
attach(dragons)
head(dragons)
...
## This is so that they have a mean of zero ("centering) and standard deviation of one ("scaling")
...
## However, what about observation independence? Are our data independent? (number 1)
...
## load data
exams<-read.csv("clipboard", header=T, sep="\t") 
attach(exams)  
names(exams) 
...
## ggplot2 is a package that you can easy change elements of a graph and build up what you want
...
## can look online for "cheatsheets" that list all the possible size and shape options
...
## load data
puzzles<-read.csv("clipboard", header=T, sep="\t") 
attach(puzzles)  
names(puzzles) 
...
## use "Awards_R" data
maths<-read.csv("clipboard", header=T, sep="\t") 
attach(maths)  
names(maths) 
...
a <- ggplot(maths, aes(x=ProgType, y=MathScore)) + 
  geom_boxplot()
## I am assigning the boxplot to "a" here, so that I can add to it without having to write it all out again
# Rotate the box plot
a + coord_flip()
...
p
##adding titles 
p<-p+labs(title="Math scores of different programmes",x="Programme Type", y = "Math Score")
p
...
## loading data
bulbs<-read.csv("clipboard", header=T, sep="\t") 
attach(bulbs)  
names(bulbs) 
...
##ggplot boxplot
a<-ggplot(bulbs, aes(x=Type, y=Price, fill=Type)) +
  geom_boxplot()
...
income.graph<-ggplot(income.data, aes(x=income, y=happiness))+
  geom_point()
income.graph
##this code says using the income.data, plot a graph with the stated x and y variable
##geom_point is used in ggplot to create scatter plots, you can add to it to change colour, shape or if you had different groups they can be represented in different colours/shapes
income.graph.2<-ggplot(income.data, aes(x=income, y=happiness))+
  geom_point(colour="red")
income.graph.2
## 2. Add the linear regression line to the plotted data and 3. equation for regression line
## using geom_smooth and lm as method to create the line, this will add the line of linear regression as well as the standard error of the estimate (in this case +/- 0.01) as a light grey stripe
income.graph <- income.graph + geom_smooth(method="lm", col="black")
income.graph
## 4. Make the graph ready for publication
## can add style parameters using theme_bw() and making custom labels using labs()
income.graph +
  theme_classic() +
